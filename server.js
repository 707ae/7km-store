...
app.get('/callback', async (req,res)=>{
  try{
    ...
    res.status(500).render('error', {title:'خطأ تسجيل الدخول', message:'تأكد من Client Secret و Redirect URI في ملف .env و Discord Developer Portal.'});
  }
});
...
app.get('/download/:id', requireLogin, async (req,res)=>{
  ...
  if(!isOwner(req)){
    ...
    if(already) return res.status(403).render('error', {title:'تم التحميل مسبقًا', message:'هذا السكربت مسموح تحميله مرة واحدة فقط.'});

    let roles = [];
    try{ roles = await getMemberRoles(req.session.user.id); }
    catch(e){ return res.status(503).render('error', {title:'الخدمة غير متاحة', message:'تعذر التحقق من رتبتك في الديسكورد. تأكد أن البوت داخل السيرفر ويملك صلاحية قراءة الأعضاء.'}); }

    if(!roles.includes(script.roleId)) return res.status(403).render('error', {title:'لا تملك الرتبة', message:'هذا السكربت غير متاح لحسابك.'});
  }

  const filePath = path.join(UPLOAD_DIR, script.file);
  if(!fs.existsSync(filePath)) return res.status(404).render('error', {title:'الملف مفقود', message:'ملف السكربت غير موجود داخل uploads.'});

  if(!isOwner(req)){
    db.downloads.push({userId:req.session.user.id, scriptId:script.id, at:new Date().toISOString()});
    writeDB(db);
  }
...
app.get('/admin', requireLogin, requireOwner, (req,res)=>{
  const db = readDB();
  res.render('admin', {scripts: db.scripts || [], downloads: db.downloads || []});
});
...
app.post('/admin/scripts', requireLogin, requireOwner, upload.single('file'), (req,res)=>{
  const db = readDB();
  if(!req.file) return res.status(400).render('error', {title:'ملف ناقص', message:'ارفع ملف ZIP.'});
  const item = {
